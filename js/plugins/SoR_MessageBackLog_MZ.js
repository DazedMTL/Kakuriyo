...
// ---------------------------------------------------------------------------
// Latest version v1.41 (2022/01/30)
//=============================================================================
/*:ja
@plugindesc ＜メッセージバックログ＞ v1.41
@author 蒼竜
@target MZ
...
*/
/*:
@plugindesc <Message Backlog> v1.41
@author Soryu
@target MZ
...
(function () {
    ...
    const LogtypeTab_Xpadd = Number(Param['LogtypeTab_Xpadd']) || '';
    const LogtypeTab_Ypadd = Number(Param['LogtypeTab_Ypadd']) || '';
    ...
    Game_Actor.prototype.displayLevelUp = function (newSkills) {
        if (!EnableAccumulate_BattleMessage) SystemLogAccumulationFlag = false;
    ...
    Scene_MSGBackLog.prototype.createLogSelectable = function () {
        this._LogListIDWindow = new Window_LogListSelection(new Rectangle(Graphics.width * 0.5 + LogtypeTab_Xpadd, LogtypeTab_Ypadd, 296, 50));
    ...
    LogMesWindow.prototype.DrawMessageTexts = function (h) {
        ...
        for (let l = 0; l < strings.length; l++) {
            ...
            this.processAllText(textState);
            th += this.calcTextHeight(textState);
    ...
    Window_LogListSelection.prototype.refreshCursor = function () {
        if (this._cursorAll) {
            ...
            this.setCursorRect(rect.x + 4, rect.y + 4, rect.width - 8, rect.height - 8);
        } else {
