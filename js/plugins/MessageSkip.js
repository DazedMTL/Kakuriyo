...
(function () {
    ...
    Game_Message.prototype.setSkipFlg = function (value) {
        ...
        if (this._skipFlg) {
            ...
            // CE1293並列処理イベントでこのプラグインの状態を取得するため。
            // 誤判定防止のため、監視スイッチは100ms後にセットする。
            // 改変者yuz-co-show
            window.setTimeout(function () { $gameSwitches.setValue(1293, true) }, 100);
        }
    ...
    Window_Message.prototype.isTriggered = function () {
        ...
        // ボイス再生中なら進めないためボイス再生中フラグがOFFの場合のみ進行（改変者yuz-co-show）
        if (this.messageAuto() && this._messageAutoCount <= 0 && !$gameSwitches.value(521)) {
            this.initializeMessageAutoCount();
            return true;
        }
